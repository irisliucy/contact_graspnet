...
def inference(global_config, checkpoint_dir, input_paths, K=None, local_regions=True, skip_border_objects=False, filter_grasps=True, segmap_id=None, z_range=[0.2,1.8], forward_passes=1):
    ...
    for i, p in enumerate(glob.glob(input_paths)):
        print('Loading ', p)
        ...
        grasp_pred_start_time = time.time()
        if pc_full is None:
        ...
        print("Grasp Prediction takes {} seconds".format(time.time() - grasp_pred_start_time))
        if i > 0:
            total_prediction_time.append(time.time() - grasp_pred_start_time)
        # Save results
    ...
    print('>>> Average grasp prediction time = {} seconds'.format(np.mean(total_prediction_time)))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--ckpt_dir', default='checkpoints/cgnet_checkpoints', help='Log dir [default: checkpoints/scene_test_2048_bs3_hor_sigma_001]')
